...
function CreatorExternalToolFilterProvider.postProcessRenderedPhotos( functionContext, filterContext )
	...
	for sourceRendition, renditionToSatisfy in filterContext:renditions( renditionOptions ) do
		...
		if success then
			...
			-- Now that the photo is completed and available to this filter, you can do your work on the photo here.  In this example,
			-- the renditions are passed to an external application that updates the Creator metadata
...
function CreatorExternalToolFilterProvider.shouldRenderPhoto( exportSettings, photo )
	...
	-- This function should return true; photo included in export, or false.
	-- Photo should be removed from the export.
	...
	-- Now that the user has filled in this section we now need to filter through the images and only choose the ones that match
